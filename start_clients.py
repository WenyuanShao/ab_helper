...
class client(object):
    def __init__(self, k, https_port, nb_requests, core):
...
def start_clients(k, https_port, nb_requests, nb_clients):
    client_list=[]
    for _ in range(nb_clients):
        nc = increment_core_num(0)
        client_list.append(client(k, https_port, nb_requests, nc))
        if https_port > 0:
            https_port += 1
    return client_list
...
if __name__ == '__main__':
    ...
    client_list = start_clients(k, https_port, nb_requests, nb_clients)
